...
void OGLRenderer::SetShaderLights(Light* light) {
	Vector3 position[4];
	Vector4 colour[4];
	float radius[4];
	...
	for (int i = 0; i < 4; i++) {
		Light& l = light[i];
		position[i] = l.GetPosition();
		colour[i] = l.GetColour();
		radius[i] = l.GetRadius();
	}

	glUniform3fv(glGetUniformLocation(currentShader->GetProgram(), "lightPos"), 4, (float*)&position);
